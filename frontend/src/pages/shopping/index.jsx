...
import {
  Box, container,CardContent,
  CardMedia,
  CardActionArea,Button,
  Typography,Accordion,
AccordionSummary,
AccordionDetails,List,FormControlLabel ,Checkbox ,Radio ,
ListItem,CircularProgress,
ListItemAvatar,
ListItemIcon,FormControl,
FormLabel,
RadioGroup,
ListItemText,Avatar ,Collapse,Slider
} from "@mui/material";
...
const Shopping = () => {
  ...
  const colors = tokens(theme.palette.mode);
    console.log(colors.greenAccent[500])
  ...
  return (
  <Box className={`shopping mt-[80px] md:mt-[120px] lg:mt-[160px]`}>
      ...
      <Box className={`container  mx-auto  my-[40px] space-4`} >
          <Box className={`flex flex-col lg:flex-row space-4`}>
            <Box  className={`w-full bg-transparent lg:w-[25%] lg:rounded-lg mx-2  `}>
                
                    <Collapse in={isNoneMobile || openCategory} timeout="auto" unmountOnExit className={``}>
                <Box backgroundColor={colors.primary[400]} className={` w-full  p-4 rounded-md lg:mb-4`}>
                      <Typography
                            variant="h1"
                        ...
                        {isFetchingAllCategory?null:(
                            
                        allCategory.map((category, index) =>(
                            
                                    <Accordion className={`bg-transparent p-4 rounded-md`} defaultExpanded={index===0}>
                                        <AccordionSummary className={`flex items-center space-x-2`} expandIcon={<ExpandMoreIcon />}>
                                                    <Avatar>
                                                        <BeachAccessIcon />
                                                    </Avatar>
                                            <Typography color={colors.greenAccent[500]} className="ml-2" variant="h5">
                    ...
                    <Collapse in={isNoneMobile || openFilter} timeout="auto" unmountOnExit className={``}>
                       <Box backgroundColor={colors.primary[400]} className=" w-full  p-4 rounded-md">
                           <Typography
                            variant="h1"
                            color={colors.grey[100]}
                            fontWeight="bold"
                            ...
                            <Box className={`mb-4`}>
                                <FormControl>
                                        ...
                                        <Box className={`flex justify-center`}>
                                            <Box>
                                                <Checkbox
                                                icon={<PaletteOutlinedIcon fontSize="large" sx={{color:"red"}} />}
                                                checkedIcon={<PaletteIcon fontSize="large" sx={{color:"red"}}  />}
                                                />
                                            </Box>
                                            <Box>
                                                <Checkbox
                                                icon={<PaletteOutlinedIcon fontSize="large" sx={{color:"blue"}} />}
                                                checkedIcon={<PaletteIcon fontSize="large" sx={{color:"blue"}} />}
                                                />
                                            </Box>
                                            <Box>
                                                <Checkbox
                                                icon={<PaletteOutlinedIcon fontSize="large" sx={{color:"green"}} />}
                                                checkedIcon={<PaletteIcon fontSize="large" sx={{color:"green"}} />}
                                                />
                                            </Box>
                                        </Box>
                                </FormControl>
            ...
            <Box backgroundColor={colors.primary[400]} className={` w-full lg:w-[75%] lg:rounded-lg p-4 mx-2 `} >
                    ...
                    <Box className={`flex m-4 py-[20px] justify-between`}>
                        ...
                        {!isNoneMobile && (
                            <Box>
                            <Button variant="text" sx={{color:colors.grey[100]}} endIcon={openCategory ? <ExpandLess color={colors.grey[100]} /> : <ExpandMore color={colors.grey[100]} />} onClick={()=>setOpenCategory(!openCategory)}>
                                Category
                            </Button>
                            <Button variant="text" sx={{color:colors.grey[100]}} endIcon={openFilter ? <ExpandLess color={colors.grey[100]} /> : <ExpandMore color={colors.grey[100]} />} onClick={()=>setOpenFilter(!openFilter)}>
                                Filter
                            </Button> 
                            </Box>
                        )}
